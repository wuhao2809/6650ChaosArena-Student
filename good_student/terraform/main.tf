...
# ---------------------------------------------------------------------------
# IAM — ECS task role (DynamoDB + S3 access)
# ---------------------------------------------------------------------------
...
resource "aws_lb" "main" {
  ...
  # Increase idle timeout to accommodate streaming 100 MB uploads (S15).
  # Default is 60 s; large uploads may take longer to stream through to S3.
  idle_timeout = 300
}
...
resource "aws_ecs_task_definition" "main" {
  ...
  # 1 vCPU / 2 GB — generous enough to handle concurrent 100 MB streaming
  # uploads without OOM.  Because we stream rather than buffer, peak heap
  # per upload is small (pipe buffer only), so this headroom covers many
  ...
  container_definitions = jsonencode([{
    name  = "app"
    image = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/${var.app_name}:${var.image_tag}"
